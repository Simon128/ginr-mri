...
class INRMetricsHook(Hook):
    ...
    def post_validation_epoch(
        ...
        **kwargs
        ...
        return {
            "inr_metric": {
                **{k: torch.mean(torch.tensor(v)) for k, v in self.val_loss_dict_stack.items()},
                **{k: torch.mean(torch.tensor(v)) for k, v in self.val_metrics_stack.items()}
            }
        }
    ...
    def post_training_epoch(
        ...
        **kwargs
        ...
        return {
            "inr_metric": {
                **{k: torch.mean(torch.tensor(v)) for k, v in self.train_loss_dict_stack.items()},
                **{k: torch.mean(torch.tensor(v)) for k, v in self.train_metrics_stack.items()}
            }
        }
